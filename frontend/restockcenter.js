...
async function annotateAIDemandTags() {
  ...
  await Promise.all(rows.map(async (row) => {
    ...
    if (!itemIDCell || !itemNameCell) return;
    ...
    Array.from(requestQtyCell.querySelectorAll('.ai-demand-tag, .ml-trend-tag')).forEach(tag => tag.remove());
    ...
    try {
      ...
      if (!user) return;
      const mlData = await getInventoryTrendPrediction(user.uid, itemID);
      if (mlData && mlData.recommendation) {
        let mlTag = document.createElement('span');
        mlTag.className = 'ml-trend-tag';
        mlTag.style.marginLeft = '8px';
        mlTag.textContent = mlData.recommendation;
        mlTag.title = mlData.reason || "";
        mlTag.style.color = mlData.recommendation === "Trend ↑" ? "#1db954" : "#d9534f";
        mlTag.style.fontWeight = "bold";
        mlTag.style.cursor = "help";
        requestQtyCell.appendChild(mlTag);
      }
      // If no mlData or no recommendation, do nothing (no tag, no loader)
    } catch {
      // Do not show anything if error
    }
  }));
