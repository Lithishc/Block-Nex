...
async function getSeasonalDemand(itemName) {
  try {
    const response = await fetch(`https://us-central1-smartsupplychain-86c30.cloudfunctions.net/api/chatgpt-seasonal-demand?item=${encodeURIComponent(itemName)}`, { cache: "no-store" });
...
async function getMarketDemand(itemName) {
  ...
  try {
    const response = await fetch(`https://us-central1-smartsupplychain-86c30.cloudfunctions.net/api/chatgpt-seasonal-demand?item=${encodeURIComponent(itemName)}&type=market`, { cache: "no-store" });
    if (!response.ok) return null;
    return await response.json();
  } catch (err) {
...
document.getElementById('show-demand-btn').onclick = async function() {
  ...
  try {
    ...
    const response = await fetch("https://us-central1-smartsupplychain-86c30.cloudfunctions.net/api/chatgpt-seasonal-demand?item=all&type=list", {
      method: "POST",
